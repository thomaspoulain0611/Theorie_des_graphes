...
void menu( std::string& commande, std::string& fichier, std::string& fichierpoids, int& nom, int& poids, int& cdn, int& cd,int& cvp, int& cpn, int& cp,int& ci, int& ciN, int& cia, int& cian, std::vector<int>& id, std::vector<int>& id2, int& taille, int& taille2, int& idArete, int& idArete2)
{
    ...
    std::cout<<std::endl<<"exit"<<std::endl<<"dessiner"<<std::endl<<"changer fichier poids"<<std::endl<<"afficher (graphe console)"<<std::endl<<"afficher centralite (console)"<<std::endl<<"afficher/enlever poids"<<std::endl<<"afficher/enlever noms"<<std::endl<<"indice centralite degre normalise"<<std::endl<<"indice centralite degre"<<std::endl<<"indice centralite vecteur propre"<<std::endl<<"incide centralite proxi normalise"<<std::endl<<"indice centralite proxi"<<std::endl<<"indice centralite intermediarite"<<std::endl<<"indice cia"<<std::endl<<"indice cian"<<std::endl<<"indice centralite intermediarite normalise"<<std::endl<<"enlever centralite"<<std::endl<<"vulnerabilite connexe"<<std::endl<<"vulnerabilite indice"<<std::endl<<"save (dans un fichier)"<<std::endl<<std::endl<<std::endl<<">";
    ...
    else if(commande=="dessiner")
    {
        ...
        cian=0;
        nom=0;
        a.dessinerTous(nom, poids, cdn, cd, cvp, cp, cpn, ci, ciN, cia, cian);
        SetConsoleTextAttribute(hConsole, 14);
    ...
    else if (commande=="indice cia")
    {
        ...
        a.dessinerTous(nom, poids, cdn, cd, cvp, cp, cpn, ci, ciN,cia,cian);
        SetConsoleTextAttribute(hConsole, 14);
        std::cout<<std::endl<<"Les indices de centralite intermediarite arete non normalises ont ete affiches avec succes "<<std::endl;
        std::cout<<std::endl<<"Le sommet marque est celui comportant le plus haut indice de centralite intermediarite arete non normalise"<<std::endl;
    }
    else if (commande=="indice cian")
    {
        cian=1;
        a.dessinerTous(nom, poids, cdn, cd, cvp, cp, cpn, ci, ciN,cia,cian);
        SetConsoleTextAttribute(hConsole, 14);
        std::cout<<std::endl<<"Les indices de centralite intermediarite arete normalises ont ete affiches avec succes "<<std::endl;
        std::cout<<std::endl<<"Le sommet marque est celui comportant le plus haut indice de centralite intermediarite arete normalise"<<std::endl;
    }
...
int main()
{
    ...
    do
    {
        menu(commande, fichier,fichierpoids,nom, poids,cdn, cd,cvp, cpn, cp,ci,ciN,cia, cian ,id, id2, taille, taille2, idArete, idArete2);
